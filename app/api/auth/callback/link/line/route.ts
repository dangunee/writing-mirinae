...
/**
 * OAuth link callback — LINE identity attached to current session user.
 */
export async function GET(request: Request) {
  ...
  try {
    ...
    if (error) {
      console.error("auth_link_line_callback_exchange_failed", error.message);
    ...
    return NextResponse.redirect(`${origin}/writing/app/settings?linked=line`);
  } catch (e) {
    console.error("auth_link_line_callback_failed", e);
    return NextResponse.redirect(`${origin}/writing/app/settings?link_error=oauth`);
  }
}
