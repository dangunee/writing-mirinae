-- mirinae.jp platform + writing app (first consumer)
-- Shared identity & commerce live in public; app domain data in schema "writing".
-- Quiz / ondoku add their own schemas later without coupling to writing tables.
...
-- Lifecycle of a purchased writing course (user schedule + progress)
...
-- Per session slot (10 per course): schedule gate + completion
...
-- Student submission pipeline (distinct from correction_status — see corrections)
...
-- Teacher correction visibility: draft = internal only; published = student APIs may return
...
-- -----------------------------------------------------------------------------
-- Shared platform: identity (single account across /writing, /quiz, /ondoku)
-- -----------------------------------------------------------------------------
CREATE TABLE users (
...
COMMENT ON TABLE users IS 'Platform-wide identity; one registration for all mirinae.jp apps. Resolve id from server session only.';
...
COMMENT ON COLUMN password_reset_tokens.token_hash IS 'Store only a hash; one-time use via used_at; 15min expiry enforced in app + expires_at.';
CREATE INDEX idx_password_reset_tokens_token_hash ON password_reset_tokens (token_hash);

-- -----------------------------------------------------------------------------
-- Platform catalog: one row per sellable product (per app)
-- -----------------------------------------------------------------------------
...
CREATE INDEX idx_products_app ON products (app);
CREATE INDEX idx_products_active ON products (app) WHERE active;
...
CREATE INDEX idx_entitlements_app_status ON entitlements (app, status);
...
CREATE INDEX idx_writing_courses_user_id ON writing.courses (user_id);
CREATE INDEX idx_writing_courses_status ON writing.courses (status);
...
CREATE INDEX idx_writing_sessions_course_id ON writing.sessions (course_id);
CREATE INDEX idx_writing_sessions_course_unlock ON writing.sessions (course_id, unlock_at);
CREATE INDEX idx_writing_sessions_status ON writing.sessions (course_id, status);

CREATE TABLE writing.submissions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_writing_submissions_user_status ON writing.submissions (user_id, status);
CREATE INDEX idx_writing_submissions_course_status ON writing.submissions (course_id, status);
CREATE INDEX idx_writing_submissions_submitted_at ON writing.submissions (submitted_at)
  WHERE submitted_at IS NOT NULL;

-- Stricter rule: one active pipeline per user across all writing courses (not per course)
CREATE UNIQUE INDEX writing_submissions_one_active_pipeline_per_user
...
CREATE INDEX idx_writing_corrections_teacher_id ON writing.corrections (teacher_id);
CREATE INDEX idx_writing_corrections_status ON writing.corrections (status);
CREATE INDEX idx_writing_corrections_published_at ON writing.corrections (published_at)
  WHERE published_at IS NOT NULL;

CREATE TABLE writing.fragments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_writing_fragments_correction_id ON writing.fragments (correction_id);
CREATE INDEX idx_writing_fragments_category ON writing.fragments (category);

-- Scores nullable while correction is draft; required before publish (enforced in trigger)
CREATE TABLE writing.evaluations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  submission_id uuid NOT NULL UNIQUE REFERENCES writing.submissions (id) ON DELETE CASCADE,
...
-- -----------------------------------------------------------------------------
-- Publish rules: evaluation complete + sync submission + session
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Submissions: course sync, owner, immutable content
-- -----------------------------------------------------------------------------
